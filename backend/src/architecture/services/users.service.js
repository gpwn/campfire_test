...
class UsersService {
  ...
  findDup = async (query) => {
    ...
    if (prop === "email") {
      ...
      const user = await this.usersRepository.findOneUserByUserName(value);
      if (user) {
  ...
  logIn = async (email, password) => {
    const hashValue = hash(password);
    const user = await this.usersRepository.findUserByAuth(email, hashValue);
  ...
  updateUser = async (userId, userName, profileImg) => {
    ...
    if (userName === user.userName && profileImg === null) {
